...
| Pixel-to-voxel reconstruction | Partial recorded-data baseline | RGB-D ray projection, synthetic frames, strict P5/P6 RGB-D loading, calibration binding, and per-frame SVO fusion. | Real capture adapters beyond PNM, depth uncertainty, and dense multiview-only reconstruction. |
...
| Visual odometry | Partial | Rigid motion fit for supplied 3D correspondences plus dependency-free RGB-D image gradient features, local patch matching, median displacement filtering, and 3D pose fitting. | Robust descriptors, RANSAC, photometric calibration, real camera poses, loop closure, and measured datasets. |
